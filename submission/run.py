...
def load_classifiers():
    """
    Загружает ансамбль моделей CatBoost со всех доступных фолдов (.cbm).
    ...
    if fold_models:
        print(f"[инфо] Загрузка ансамбля из {len(fold_models)} моделей CatBoost...")
        ...
        if single_model_path.exists():
            ...
            raise FileNotFoundError(f"Файлы весов CatBoost (.cbm) не найдены в {WEIGHTS_DIR}")
...
def predict(classifiers, features):
    ...
    по ансамблю моделей CatBoost для метрики Macro PR-AUC.
    """
